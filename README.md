...
| `TEKTON_RESULTS_INSECURE` | `false` | `fetch-tekton-records.sh` | When `true` (lowercase only), disables Results certificate verification and logs a warning; emergency compatibility use only |
...
**Note:** `TEKTON_RESULTS_INSECURE=true` takes precedence over
`TEKTON_RESULTS_CA_PATH` — the CA file is ignored when insecure mode is
enabled.
