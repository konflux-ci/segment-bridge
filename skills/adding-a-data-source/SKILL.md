...
`scripts/tekton-main-job.sh` — wire the new fetch behind a `FETCH_<NAME>` toggle
(default on). Default-on keeps existing clusters collecting the new source;
the toggle is the opt-out (noisy or broken CRD, debugging) without a rebuild.
Add it when wiring a production fetch so every source has the same control:
...
4. Document `FETCH_<NAME>` (default `true`) in `README.md`. Only the literal
   value `false` (case-insensitive) disables a source.
