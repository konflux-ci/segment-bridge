...
write_cursor() {
  ...
  # offset (e.g. non-GNU date on some local dev machines). Cursor
  # persistence is best-effort and must never abort the whole script.
  local backed_off_cursor
  if ! backed_off_cursor=$(date -u -d "${new_cursor} - 1 second" +%Y-%m-%dT%H:%M:%SZ 2>/dev/null); then
    echo "fetch-tekton-records.sh: could not compute cursor overlap offset (date -d unsupported); using exact cursor" >&2
