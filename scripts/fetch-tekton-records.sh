...
if ! [[ "$TEKTON_LIMIT" =~ ^[1-9][0-9]*$ ]]; then
  echo "ERROR fetch-tekton-records.sh: TEKTON_LIMIT must be a positive integer, got '${TEKTON_LIMIT}'" >&2
  exit 1
fi
#
...
if ! [[ "$TEKTON_MAX_PAGES" =~ ^[1-9][0-9]*$ ]]; then
  echo "ERROR fetch-tekton-records.sh: TEKTON_MAX_PAGES must be a positive integer, got '${TEKTON_MAX_PAGES}'" >&2
  exit 1
fi
#
...
#
# Emergency compatibility switch. Accepts 'true' or 'false' (lowercase only).
# When 'true', disables TLS certificate verification and logs a warning.
...
CURL_TLS_ARGS=()
# Note: TEKTON_RESULTS_INSECURE=true takes precedence over TEKTON_RESULTS_CA_PATH.
if [[ "$TEKTON_RESULTS_INSECURE" == "true" ]]; then
  echo "WARN fetch-tekton-records.sh: TEKTON_RESULTS_INSECURE=true disables TLS certificate verification for Tekton Results" >&2
  CURL_TLS_ARGS+=(--insecure)
elif [[ -n "$TEKTON_RESULTS_CA_PATH" ]]; then
  if [[ ! -r "$TEKTON_RESULTS_CA_PATH" ]] || [[ ! -f "$TEKTON_RESULTS_CA_PATH" ]] || [[ ! -s "$TEKTON_RESULTS_CA_PATH" ]]; then
    echo "ERROR fetch-tekton-records.sh: TEKTON_RESULTS_CA_PATH is not a readable non-empty file: ${TEKTON_RESULTS_CA_PATH}" >&2
    exit 1
  fi
...
while true; do
  ...
  if ! RESPONSE=$(curl --silent --show-error --fail "${CURL_TLS_ARGS[@]}" --header "Authorization: Bearer $TOKEN" "$URL"); then
  ...
  if ! echo "$RESPONSE" | jq -c --arg cursor "${CURSOR}" -f "$SELFDIR/jq/filter-pipelineruns.jq"; then
    echo "ERROR fetch-tekton-records.sh: failed to parse/filter API response on page ${PAGE_COUNT}" >&2
    exit 1
  fi
  ...
  if ! PAGE_TOKEN=$(echo "$RESPONSE" | jq -r '.nextPageToken // empty'); then
    echo "ERROR fetch-tekton-records.sh: failed to parse nextPageToken from API response on page ${PAGE_COUNT}" >&2
    exit 1
  fi
...
# Only persist the cursor when pagination completed cleanly (reached the
# end of the result set or hit the cursor boundary). Persisting after a
# truncated run (HIT_MAX_PAGES) would permanently exclude records on
# unfetched pages from future runs, since the cursor filter excludes
# anything at or before it.
if [[ "$HIT_MAX_PAGES" != "true" ]]; then
  write_cursor "$MAX_CREATE_TIME"
else
  echo "WARN fetch-tekton-records.sh: cursor NOT advanced — pagination was truncated by TEKTON_MAX_PAGES (${TEKTON_MAX_PAGES}); unfetched pages will be retried next run" >&2
fi
