...
func TestFetchTektonRecordsAllowsExplicitInsecureMode(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Len(t, nonEmptyLines(out), 2)
	assert.Contains(t, string(stderr), "WARN fetch-tekton-records.sh: TEKTON_RESULTS_INSECURE=true disables TLS certificate verification")
	require.NotNil(t, *captured)
}
...
func TestFetchTektonRecordsRejectsUnreadableCAFile(t *testing.T) {
	...
	require.Error(t, err)
	assert.Contains(t, string(stderr), "TEKTON_RESULTS_CA_PATH is not a readable non-empty file")
}
...
// TestFetchTektonRecordsHTTPErrors verifies that HTTP error responses
// (e.g. 500, 403) cause the script to exit non-zero with a clear ERROR
// diagnostic on stderr, thanks to curl --fail.
func TestFetchTektonRecordsHTTPErrors(t *testing.T) {
	cases := []struct {
		name       string
		statusCode int
	}{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, stderr, err := runFetchTektonWithStderr(t, baseEnv(server.URL))
			require.Error(t, err, "HTTP %d must cause script to exit non-zero", tc.statusCode)
			assert.Contains(t, string(stderr),
				"ERROR fetch-tekton-records.sh: Tekton Results API request failed",
				"HTTP %d must produce ERROR diagnostic on stderr", tc.statusCode)
		})
	}
}

// TestFetchTektonRecordsPaginationPage2HTTPError verifies that an HTTP
// error on the second page (after the first page succeeded) still causes
// the script to exit non-zero with a clear ERROR diagnostic, rather than
// only being covered for the first request.
func TestFetchTektonRecordsPaginationPage2HTTPError(t *testing.T) {
	absPage1, err := filepath.Abs("testdata/records-page1.json")
	require.NoError(t, err)

	var requestCount int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		atomic.AddInt32(&requestCount, 1)
		if r.URL.Query().Get("page_token") == "page2token" {
			http.Error(w, "Internal Server Error", http.StatusInternalServerError)
			return
		}
		data, err := os.ReadFile(absPage1)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	_, stderr, err := runFetchTektonWithStderr(t, baseEnv(server.URL))
	require.Error(t, err, "HTTP error on page 2 must cause script to exit non-zero")
	assert.Contains(t, string(stderr),
		"ERROR fetch-tekton-records.sh: Tekton Results API request failed",
		"HTTP error on page 2 must produce ERROR diagnostic on stderr")
	assert.Equal(t, int32(2), atomic.LoadInt32(&requestCount),
		"page 1 must have succeeded before page 2 failed")
}

// TestFetchTektonRecordsInvalidLimitAndMaxPages verifies that non-positive
// or non-numeric TEKTON_LIMIT / TEKTON_MAX_PAGES values are rejected before
// any HTTP request is made.
func TestFetchTektonRecordsInvalidLimitAndMaxPages(t *testing.T) {
	cases := []struct {
		name    string
		envVar  string
		value   string
		...
		{name: "TEKTON_LIMIT=0", envVar: "TEKTON_LIMIT", value: "0", wantMsg: "TEKTON_LIMIT must be a positive integer"},
		{name: "TEKTON_LIMIT=abc", envVar: "TEKTON_LIMIT", value: "abc", wantMsg: "TEKTON_LIMIT must be a positive integer"},
		{name: "TEKTON_MAX_PAGES=0", envVar: "TEKTON_MAX_PAGES", value: "0", wantMsg: "TEKTON_MAX_PAGES must be a positive integer"},
		{name: "TEKTON_MAX_PAGES=abc", envVar: "TEKTON_MAX_PAGES", value: "abc", wantMsg: "TEKTON_MAX_PAGES must be a positive integer"},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			env := baseEnv(server.URL)
			env[tc.envVar] = tc.value
			_, stderr, err := runFetchTektonWithStderr(t, env)
			require.Error(t, err, "%s must cause script to exit non-zero", tc.name)
			assert.Contains(t, string(stderr), tc.wantMsg,
				"%s must produce a clear validation error", tc.name)
		})
	}
}
