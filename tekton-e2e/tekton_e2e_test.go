...
func TestHappyPath(t *testing.T) {
	...
	assert.Equal(t, 8, len(allEvents), "Expected 8 events (6 from PipelineRuns + 2 from Konflux operator)")
...
func TestEmptyPipeline(t *testing.T) {
	...
	assert.Equal(t, 2, len(allEvents), "Expected 2 events from Konflux operator")
...
// TestMissingAuthToken tests error handling for missing authentication token
func TestMissingAuthToken(t *testing.T) {
	...
	assert.NotEqual(t, 0, exitCode, "Pipeline should fail without authentication token")
	assert.Contains(t, output, "No authentication token available", "Error message should mention authentication")
}
...
func TestTaskRunFiltering(t *testing.T) {
	...
	assert.Equal(t, 4, len(allEvents), "Expected 4 events (2 from PipelineRun + 2 from operator)")
...
func TestBatchSplitting(t *testing.T) {
	...
	// Should have 42 events total (40 from PipelineRuns + 2 from Konflux operator)
	assert.Equal(t, 42, len(allEvents), "All events should be delivered across batches")
...
func TestNamespaceAnonymization(t *testing.T) {
	...
	pipelineEvents := []SegmentEvent{}
	for _, event := range allEvents {
		if event.Event == "PipelineRun Started" || event.Event == "PipelineRun Completed" {
